...
export function HomeHero({ sx, ...other }) {
  ...
  const renderHeading = (
    <MInview>
      ...
      >
        <Box sx={{ width: 1, opacity: 0.24 }}>Powerful Applications to Scale &</Box>
        Grow Your
        <Box
          component={m.span}
        ...
        {['product-1', 'product-2', 'product-3', 'product-4'].map((product) => (
          <MInview key={product}>
            <Box
              component="img"
              alt={product}
              src={`${CONFIG.site.basePath}/assets/icons/products/${product}.svg`}
              sx={{ width: 30, height: 30, borderRadius: 15 }}
            />
          </MInview>
        ))}
      </Stack>
